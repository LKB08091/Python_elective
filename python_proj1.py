    ...
    try:
        while True: #keep reading, unless keyboard is pressed
            ...
            if result.is_valid(): #print datetime & sensor values
                ...
                result_string=f"{result.temperature:.2f}*C,{result.humidity:2f}%"
...
def send_message(key):
    ...
    n = product_list[key].name
    q = product_quantity[key].quantity
    message = f"A purchase was made on {t}: item {key} {n}. New quantity: {q}"
